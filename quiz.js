document.addEventListener('DOMContentLoaded', function() {
    ...
    function showQuestion() {
        ...
        if (currentQuestion.type === "multiple-choice") {
            ...
            inputContainer.classList.add('mb-3', 'mt-3', 'd-flex', 'align-items-center', 'justify-content-center', 'flex-wrap');
            
            // Create input group for each answer part
            const answerParts = currentQuestion.answer.length;
            const inputGroups = [];
            
            for (let i = 0; i < answerParts; i++) {
                const inputGroup = document.createElement('div');
                inputGroup.classList.add('input-group', 'mb-2', 'me-2');
                
                if (currentQuestion.frontUnit && i === 0) {
                    const frontUnitSpan = document.createElement('span');
                    frontUnitSpan.classList.add('input-group-text');
                    frontUnitSpan.textContent = currentQuestion.frontUnit[0];
                    inputGroup.appendChild(frontUnitSpan);
                }
                ...
                if (currentQuestion.unit && i < currentQuestion.unit.length) {
                    const unitSpan = document.createElement('span');
                    unitSpan.classList.add('input-group-text');
                    unitSpan.textContent = currentQuestion.unit[i];
            ...
            submitBtn.addEventListener('click', () => {
                const userAnswers = inputGroups.map(input => input.value.trim());
                checkFillInAnswer(userAnswers);
            });
    ...
    function checkFillInAnswer(userAnswers) {
        // If an answer is already submitted, do nothing
        if (selectedOption !== null) return;

        selectedOption = userAnswers;
        const currentQuestion = questions[currentQuestionIndex];
        const inputs = document.querySelectorAll('input[id^="answer-input-"]');
        const submitBtn = inputs[0].closest('.input-group').parentElement.lastElementChild;
        ...
        // Apply formatting to the inputs based on correctness
